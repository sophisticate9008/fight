...
__plugin_cd_limit__ = {
    ...
    "rst": "请等待60秒",            # 回复的话，为None时不回复，可以添加[at]，[uname]，[nickname]来对应艾特，用户群名称，昵称系统昵称
    "status": False          # 此限制的开关状态
}
__plugin_count_limit__ = {
    "max_count": 5,    # 每日次数限制数量
    "limit_type": "user",   # 监听对象，以user_id或group_id作为键来限制，'user'：用户id，'group'：群id
    "rst": "每天只能进行五次投注，你没有机会了",            # 回复的话，为None时不回复，可以添加[at]，[uname]，[nickname]来对应艾特，用户群名称，昵称系统昵称
    ...
    async def _(bot: Bot,                                                                   
        ...
        if int(selRole) == 0:
            if(list_fight[4].isDisplayVictory == 1):
                money_add = int (money_spend / list_prob[1] * 0.95)
        ...
        if int(selRole) == 1:
            if(list_fight[5].isDisplayVictory == 1):
                money_add = int (money_spend * 1 / list_prob[3] * 0.95)
                await BagUser.add_gold(uid, group, money_add)
                gold_have = await BagUser.get_gold(uid, group)
                await ready.finish( '你支持的英桀获胜,你获得{}金币,当前金币为{}'.format(money_add, gold_have))
